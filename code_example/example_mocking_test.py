...
SUM_SERVICE = "example_mocking_test.SumService"
...
class Calculator:
    def __init__(self, sum_service: SumService):
        self.sum_service = sum_service

    def make_sum(self, a, b):
        return self.sum_service.sum(a, b)
...
def test_calculator_without_mock():
    ...
    assert calculator.make_sum(1, 2)
...
def test_calculator_with_magic_mock():
    ...
    # Calling a non-existent method this
...
def test_with_mock_patch():
    ...
    with patch(SUM_SERVICE) as mock_sum_service:
...
def test_with_mock_patch_and_spec():
    with patch(SUM_SERVICE, spec=SumService) as mock_sum_service:
        mock_sum_service.sum.return_value = 3
...
def test_with_mock_patch_and_autospec():
    with patch(SUM_SERVICE, autospec=True) as mock_sum_service:
        mock_sum_service.sum.return_value = 3
